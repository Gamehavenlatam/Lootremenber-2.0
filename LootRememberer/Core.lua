...
function LootRememberer:GetCharacterProfileKey()
    return UnitName("player") .. " - " .. GetRealmName()
end

function LootRememberer:ApplyProfileFromSettings()
    if self.db.global.perCharacter then
        self.db:SetProfile(self:GetCharacterProfileKey())
    else
...
function LootRememberer:OnInitialize()
    ...
    self.db = LibStub("AceDB-3.0"):New("LootRemembererDB", DB_DEFAULTS, true)
    self:ApplyProfileFromSettings()
...
function LootRememberer:GetChatFrameOptions()
    local options = {}
    for i = 1, NUM_CHAT_WINDOWS do
        ...
        if frame and frame:IsShown() then
            ...
            options["ChatFrame" .. i] = name
        end
    end
    return options
end

function LootRememberer:GetChatFrame()
    local frame = _G[self.db.global.chatFrameName]
...
function LootRememberer:PrintToChat(message)
    local frame = self:GetChatFrame()
    frame:AddMessage("|cff33ff99Loot Rememberer:|r " .. message)
end

function LootRememberer:InitOptions()
    local options = {
        type = "group",
        name = "Loot Rememberer",
        args = {
            perCharacter = {
                type = "toggle",
                order = 1,
                name = "Configuración y lista por personaje",
                desc = "Si está activado, cada personaje tiene su propia lista de decisiones de loot guardadas. Si está desactivado, la lista se comparte entre todos los personajes de la cuenta.",
                width = "full",
                get = function () return self.db.global.perCharacter end,
                set = function (_, value)
                    self.db.global.perCharacter = value
                    self:ApplyProfileFromSettings()
                    if self.guiContainer and self.guiContainer:IsVisible() then
            ...
            minimapIcon = {
                type = "toggle",
                order = 2,
                name = "Mostrar botón de minimapa",
                desc = "Muestra u oculta el botón de Loot Rememberer en el minimapa.",
                width = "full",
                get = function () return not self.db.global.minimap.hide end,
                set = function (_, value)
                    self.db.global.minimap.hide = not value
                    if value then
                        LibStub("LibDBIcon-1.0"):Show("LootRememberer")
                    else
                        LibStub("LibDBIcon-1.0"):Hide("LootRememberer")
                    end
                end,
            },
            openList = {
                type = "execute",
                order = 3,
                name = "Abrir lista de registros",
                desc = "Abre la ventana con el historial de decisiones guardadas.",
                func = function () self:SlashProcessor() end,
            },
            chatFrame = {
                type = "select",
                order = 4,
                name = "Ventana de chat para los mensajes",
                desc = "Elige a qué ventana de chat (por ejemplo si usas doble ventana como ElvUI) se envían los mensajes de Loot Rememberer.",
                width = "full",
                values = function () return self:GetChatFrameOptions() end,
                get = function () return self.db.global.chatFrameName end,
                set = function (_, value)
                    self.db.global.chatFrameName = value
                    self:PrintToChat("Los mensajes ahora se mostrarán en esta ventana.")
                end,
            },
        },
    }

    LibStub("AceConfig-3.0"):RegisterOptionsTable("LootRememberer", options)
    self.optionsFrame = LibStub("AceConfigDialog-3.0"):AddToBlizOptions("LootRememberer", "Loot Rememberer")
end

function LootRememberer:InitMinimapIcon()
    local ldb = LibStub("LibDataBroker-1.1"):NewDataObject("LootRememberer", {
        type = "launcher",
        text = "Loot Rememberer",
        icon = "Interface\\Icons\\INV_Misc_Bag_10_Blue",
        OnClick = function (_, button)
            if button == "RightButton" then
                LibStub("AceConfigDialog-3.0"):Open("LootRememberer")
            else
                self:SlashProcessor()
            end
        end,
        OnTooltipShow = function (tooltip)
            tooltip:AddLine("Loot Rememberer")
            tooltip:AddLine("Click izquierdo: abrir lista de registros")
            tooltip:AddLine("Click derecho: abrir configuración")
        end,
    })

    LibStub("LibDBIcon-1.0"):Register("LootRememberer", ldb, self.db.global.minimap)
end
...
function LootRememberer:RecordLootRoll(rollId, rollMode)
    ...
    local displayName = itemName and ("[" .. itemName .. "]") or itemLink
    local rollTypeName = ROLL_TYPE_NAMES[rollMode] or "?"
    self:PrintToChat(displayName .. " -> " .. rollTypeName .. " (guardado)")
...
function LootRememberer:DeferredLootRollHandler(rollId)
    ...
    local displayName = itemName and ("[" .. itemName .. "]") or itemLink
    local rollTypeName = ROLL_TYPE_NAMES[rollMode] or "?"
    self:PrintToChat(displayName .. " -> " .. rollTypeName .. " (automático)")
end
...
function LootRememberer:SlashProcessor(input)
    ...
    for i = 1, GUI_NUM_RECORDS_PER_PAGE do
        ...
        recordFrame.Update = function (_, itemLink, details)
            ...
            if not itemName then
                -- Item info not cached yet; show a fallback and retry once it's available
                itemName = itemLink:match("%[(.-)%]") or "?"
                itemQuality = itemQuality or 1
                itemTexture = itemTexture or "Interface\\Icons\\INV_Misc_QuestionMark"

                if not recordFrame.pendingItemInfoLink or recordFrame.pendingItemInfoLink ~= itemLink then
                    recordFrame.pendingItemInfoLink = itemLink
                    C_Timer.After(0.5, function ()
                        if recordFrame.itemLink == itemLink then
                            recordFrame.pendingItemInfoLink = nil
                            recordFrame.Update(nil, itemLink, details)
                        end
                    end)
                end
            end
